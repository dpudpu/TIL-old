...
public class BoardDao {
    ...
    public BoardDao(DataSource dataSource){
        ...
        this.insertAction = new SimpleJdbcInsert(dataSource) // Insert문을 자동으로 내부적으로 생성.
                .withTableName("board")
                .usingGeneratedKeyColumns("id");
    }

    public int upateCount(Long id) {
        String sql= "update board set read_count = read_count+1 where id = :id";
        Map<String, Long> map = Collections.singletonMap("id",id); // 1개일때 사용. 더 빠름
...
//        map.put("content", board.getContent());
    ...
    public List<Board> getBoards(int start, int limit) {
        String sql = "SELECT * FROM board LIMIT :start, :limit";
        RowMapper<Board> rowMapper = BeanPropertyRowMapper.newInstance(Board.class);
        Map<String, Integer> params = new HashMap();
        params.put("start",start);
        params.put("limit",limit);
        List<Board> boards = jdbc.query(sql, params, rowMapper);
